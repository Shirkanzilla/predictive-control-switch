...
def sac(env_fn, actor_fn=mlp_actor, critic_fn=mlp_critic, ac_kwargs=dict(), seed=0,
    ...
    pi_loss = tf.reduce_mean(
        alpha * logp_pi - min_q_pi + (beta - damp) * qc_pi)
    ...
    logger.setup_tf_saver(sess, inputs={'x': x_ph, 'a': a_ph},
                          outputs={'mu': mu, 'pi': pi, 'logp_a': logp_a, 'qr1': qr1, 'qr2': qr2, 'qc': qc})
    ...
    def _test_robust_and_save(coef_dic, n=100):
        for j in range(n):
            ...
            while not (d or (ep_len == max_ep_len)):
                # Take deterministic actions at test time
                o, r, d, info = test_env.step(get_action(o, False))
                ep_ret += r
                ep_cost += info.get('cost', 0)
                ep_len += 1
                ep_goals += 1 if info.get('goal_met', False) else 0
    ...
    def test_robustness():
        for mass in np.arange(start=0.01, stop=0.02, step=0.002):
    ...
    for t in range(total_steps // num_procs()):
        ...
        exploration
        '''
        xt1, yt1, _ = env.world.robot_pos()

        r *= reward_scale  # yee-haw
        c = info.get('cost', 0)
        ds = ((xt1-xt) ** 2 + (yt1-yt) ** 2) ** 0.5

        r += reward_bonus*ds

        if abs(xt) > max(env.placements_extents) or abs(yt) > max(env.placements_extents):
            d = True
            r = 0
            env.reset()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='semidynamic-v0')
    ...
    parser.add_argument('--fixed_cost_penalty', default=None, type=float)
    parser.add_argument('--cost_constraint', type=float, default=None)
    parser.add_argument('--cost_lim', type=float, default=8)
    parser.add_argument('--lr_s', type=int, default=50)
    parser.add_argument('--damp_s', type=int, default=10)
    parser.add_argument('--reward_b', type=float, default=1.0)
    parser.add_argument('--logger_kwargs_str', type=json.loads,
