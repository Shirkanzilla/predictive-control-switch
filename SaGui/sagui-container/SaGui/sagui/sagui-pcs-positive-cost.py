...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        ...
        self.rhos_buf = np.zeros(size, dtype=np.float32)
        self.costs_buf = np.zeros(size, dtype=np.float32)
        self.done_buf = np.zeros(size, dtype=np.float32)
        self.ptr, self.size, self.max_size = 0, 0, size

    def store(self, obs, act, rew, rew1, rho, next_obs, done, cost):
        self.obs1_buf[self.ptr] = obs
        self.obs2_buf[self.ptr] = next_obs
        self.acts_buf[self.ptr] = act
        self.rews_buf[self.ptr] = rew
        self.rews1_buf[self.ptr] = rew1
        self.rhos_buf[self.ptr] = rho
...
def sac(env_fn, get_logp_a_fn, get_teacher_a_fn, teacher_size, teacher_keys, classifier, device, actor_fn=mlp_actor, critic_fn=mlp_critic,  ac_kwargs=dict(), seed=0,
    ...
    with tf.variable_scope('main', reuse=True):
        ...
        _, pi2, logp_pi2, _ = actor_fn(x2_ph, a_ph, **ac_kwargs)
    ...
    epoch = 0
    point_view = True
    sampling_index = random.random()
    p_ts = 1.0
    d_p_ts = update_freq/total_steps
    rec = False
    rec_step = total_steps
    for t in range(total_steps // num_procs()):
        ...
        if rho == 1.0:
            replay_buffer_stu.store(o, a, r, r1, rho, o2, d, c)
        else:
            replay_buffer_tea.store(o, a, r, r1, rho, o2, d, c)
        ...
        if t > 0 and t % local_steps_per_epoch == 0:
            ...
            logger.log_tabular('RecoveryAtStep', rec_step)
...
if __name__ == '__main__':
    ...
    sac(lambda: gym.make(args.env), get_logp_a_fn=get_logp_a, get_teacher_a_fn=get_teacher_a, teacher_size=_teacher_size, teacher_keys=_teacher_keys, classifier=classifier, 
        actor_fn=mlp_actor, critic_fn=mlp_critic, device=device, 
